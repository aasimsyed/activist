...
test.beforeEach(async ({ page }) => {
  ...
  // Ensure page is fully interactive after navigation
...
test.describe(
  ...
  () => {
    ...
    test(
      "Organization About Page has no detectable accessibility issues",
      { tag: "@a11y" },
      async ({ page }, testInfo) => {
    ...
    test(
      "User can edit the About section",
      { tag: "@crud" },
      async ({ page }) => {
        const organizationPage = newOrganizationPage(page);
    ...
    test(
      "User can manage social links (CREATE, UPDATE, DELETE)",
      { tag: ["@crud", "@slow"] },
      async ({ page }) => {
        const organizationPage = newOrganizationPage(page);
